...
export const BookingCreateSchema = BookingCreateInputSchema.extend({
  ...
  }).min(0, "Score must be at least 0.").max(1, "Score must be at most 1."),
...
export const BookingPatchSchema = BookingUpdateInputSchema.extend({
  matchedPrice: z.number({
    invalid_type_error: "Matched price must be a number.",
  }).nonnegative("Matched price cannot be negative.").optional(),
  score: z.number({
    invalid_type_error: "Score must be a number.",
  }).min(0, "Score must be at least 0.").max(1, "Score must be at most 1.").optional(),
